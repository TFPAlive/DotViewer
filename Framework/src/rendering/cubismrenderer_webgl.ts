...
export class CubismRenderer_WebGL extends CubismRenderer {
   ...
   * @return レンダーテクスチャの枚数
   */
  public setDrawableVisible(drawableIndex: number, visible: boolean): void {
    if (visible) {
      this._hiddenDrawableIndices.delete(drawableIndex);
    } else {
      this._hiddenDrawableIndices.add(drawableIndex);
    }
  }
  ...
  public constructor(width: number, height: number) {
    ...
    this._hiddenDrawableIndices = new Set<number>();
  ...
  public drawDrawable(drawableIndex: number, rootFbo: WebGLFramebuffer): void {
    if (this._hiddenDrawableIndices.has(drawableIndex)) {
      return;
    }
    ...
    if (clipContext != null && this.isUsingHighPrecisionMask()) {
...

      {
        ...
        for (let index = 0; index < clipDrawCount; index++) {
          ...
          if (this._hiddenDrawableIndices.has(clipDrawIndex)) {
            continue;
          }
  ...
  _hiddenDrawableIndices: Set<number>;
